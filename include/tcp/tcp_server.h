...
namespace netco{
    ...
    public:
        ...
        typedef std::function<void(netco::Socket::Ptr)> conn_callback;
        
        TcpServer() : _listen_fd(nullptr),
            server_ip(nullptr),server_port(-1) {
            NETCO_LOG_FMT("%s", "constructor the tcp server finish");
        }
    ...
    private:
        ...
        /** 用于监听的socket*/
        netco::Socket::Ptr _listen_fd;
        
        /** 多线程下监听的socket队列*/
        std::vector<netco::Socket::Ptr> _multi_listen_fd;
